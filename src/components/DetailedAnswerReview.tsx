...
export const DetailedAnswerReview = ({ attemptId }: DetailedAnswerReviewProps) => {
  ...
  if (isLoading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen py-12 px-4">
        <div className="w-24 h-24 md:w-32 md:h-32 bg-gradient-to-br from-emerald-500 to-blue-600 rounded-3xl flex items-center justify-center mb-8 animate-pulse">
          <BookOpen className="w-12 h-12 md:w-16 md:h-16 text-white" />
        </div>
        <Loader2 className="w-16 h-16 md:w-20 md:h-20 animate-spin text-emerald-600 mb-6" />
        <span className="text-gray-600 text-2xl md:text-3xl text-center">
          جاري تحميل تفاصيل الإجابات...
        </span>
        <div className="w-80 max-w-full h-4 md:h-5 bg-gray-200 rounded-full overflow-hidden mt-10">
  ...
  if (!userAnswers || userAnswers.length === 0) {
    return (
      <div className="text-center py-16 px-4 min-h-screen flex flex-col justify-center">
        <div className="w-24 h-24 md:w-32 md:h-32 bg-gray-100 rounded-3xl flex items-center justify-center mx-auto mb-8">
          <AlertCircle className="w-12 h-12 md:w-16 md:h-16 text-gray-400" />
        </div>
        <h3 className="text-3xl md:text-4xl font-bold text-gray-900 mb-6">
          لا توجد بيانات متاحة
        </h3>
        <p className="text-gray-600 text-2xl md:text-3xl max-w-2xl mx-auto">
  ...
  return (
    <div className="max-w-5xl mx-auto w-full px-4 md:px-8 py-8 space-y-10 md:space-y-12" dir="rtl">
      {/* Summary Section - Even Larger Design */}
      <div className="p-6 md:p-10 bg-gradient-to-r from-emerald-50 to-blue-50 rounded-3xl border-4 border-emerald-200 shadow-xl">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-8 mb-10">
          <div className="flex flex-col md:flex-row items-center gap-6 md:gap-10">
            {scorePercentage >= 70 ? (
              <div className="w-20 h-20 md:w-28 md:h-28 bg-gradient-to-br from-emerald-500 to-green-600 rounded-3xl flex items-center justify-center">
                <Award className="w-12 h-12 md:w-16 md:h-16 text-white" />
              </div>
            ) : (
              <div className="w-20 h-20 md:w-28 md:h-28 bg-gradient-to-br from-yellow-500 to-orange-600 rounded-3xl flex items-center justify-center">
            ...
            <div className="text-center md:text-left">
              ...
              <p className="text-gray-700 text-2xl md:text-3xl mt-3">
        ...
        {/* Performance indicators - Larger Grid */}
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 md:gap-8 mb-10">
          <div className="text-center p-6 bg-emerald-100 rounded-3xl border-3 border-emerald-200">
            <div className="flex items-center justify-center gap-4 mb-4">
              ...
              <span className="text-emerald-600 text-2xl md:text-3xl">صحيحة:</span>
            </div>
          ...
          <div className="text-center p-6 bg-red-100 rounded-3xl border-3 border-red-200">
            <div className="flex items-center justify-center gap-4 mb-4">
              ...
              <span className="text-red-600 text-2xl md:text-3xl">خاطئة:</span>
            </div>
          ...
          <div className="text-center p-6 bg-blue-100 rounded-3xl border-3 border-blue-200">
            <div className="flex items-center justify-center gap-4 mb-4">
              ...
              <span className="text-blue-600 text-2xl md:text-3xl">النسبة المئوية:</span>
            </div>
        ...
        <div className={`p-6 md:p-8 rounded-3xl border-4 text-center ${
          ...
          <div className="font-extrabold text-2xl md:text-3xl flex items-center justify-center gap-4">
            {scorePercentage >= 90 && <><span className="text-4xl">🏆</span> أداء ممتاز!</>}
            {scorePercentage >= 80 && scorePercentage < 90 && <><span className="text-4xl">🌟</span> أداء جيد جداً!</>}
            {scorePercentage >= 70 && scorePercentage < 80 && <><span className="text-4xl">✅</span> أداء جيد</>}
            {scorePercentage >= 60 && scorePercentage < 70 && <><span className="text-4xl">⚠️</span> يحتاج مراجعة</>}
            {scorePercentage < 60 && <><span className="text-4xl">📚</span> يحتاج تدريب إضافي</>}
          </div>
        </div>
      </div>

      {/* Questions List - Larger Design */}
          ...
          return (
            ...
            >
              <CardContent className="p-0">
                    ...
                    <div className="flex items-start justify-between gap-5">
                      <div className="flex-1 min-w-0">
                        <div className="flex flex-wrap items-center gap-4 mb-4">
                          <h5 className="font-extrabold text-2xl md:text-3xl text-gray-900">
                            السؤال {index + 1}
                          </h5>
                          <Badge 
                            variant={userAnswer.is_correct ? "secondary" : "destructive"}
                            className="text-xl md:text-2xl px-4 py-2"
                          >
                            {userAnswer.is_correct ? '✓ صحيح' : '✗ خطأ'}
                          </Badge>
                        </div>
                        
                        <p className="text-gray-800 text-xl md:text-2xl font-bold break-words leading-relaxed">
                          {question?.question_text}
                        </p>
                      </div>
                      <div className="mt-2">
                        {isExpanded ? (
                          <ChevronUp className="w-10 h-10 md:w-12 md:h-12 text-gray-500" />
                        ) : (
                          <ChevronDown className="w-10 h-10 md:w-12 md:h-12 text-gray-500" />
                        )}
                      </div>
                    </div>

                    {/* Quick summary - Larger layout */}
                    <div className="mt-6 space-y-6 md:space-y-8">
                      <div className="flex flex-col gap-3">
                        <span className="text-gray-700 text-xl md:text-2xl">إجابتك:</span>
                        <div className={`flex items-center gap-5 p-5 md:p-6 rounded-2xl text-xl md:text-2xl ${
                          userAnswer.is_correct 
                            ? 'bg-emerald-100 text-emerald-800' 
                            : 'bg-red-100 text-red-800'
                        }`}>
                          {userAnswer.is_correct ? 
                            <CheckCircle className="w-8 h-8 md:w-10 md:h-10 flex-shrink-0" /> : 
                      ...
                      {!userAnswer.is_correct && correctOption && (
                        <div className="flex flex-col gap-3">
                          <span className="text-gray-700 text-xl md:text-2xl">الإجابة الصحيحة:</span>
                          <div className="flex items-center gap-5 p-5 md:p-6 bg-emerald-100 text-emerald-800 rounded-2xl text-xl md:text-2xl">
                            <CheckCircle className="w-8 h-8 md:w-10 md:h-10 flex-shrink-0" />
                            <span className="font-bold break-words text-left flex-1">
                ...
                {/* Expanded Content - Larger Design */}
                {isExpanded && (
                  <div className="mt-0 space-y-8 border-t-2 border-gray-200 p-6 md:p-8">
                    <h6 className="font-extrabold text-gray-900 text-2xl md:text-3xl flex items-center gap-4 mb-6">
                      <Target className="w-8 h-8 md:w-10 md:h-10 text-blue-600" />
                      الخيارات المتاحة:
                    </h6>
                      ...
                      {question?.answer_options
                        ...
                        .map((option) => {
                          ...
                          if (isCorrectAnswer) {
                            ...
                            iconElement = <CheckCircle className="w-8 h-8 md:w-10 md:h-10 text-emerald-600 flex-shrink-0" />;
                            badgeContent = (
                              <Badge variant="secondary" className="text-xl md:text-2xl bg-emerald-600 text-white px-4 py-2">
                                ✓ الإجابة الصحيحة
                              </Badge>
                            );
                          }
                          ...
                          if (isUserAnswer && !isCorrectAnswer) {
                            ...
                            iconElement = <XCircle className="w-8 h-8 md:w-10 md:h-10 text-red-600 flex-shrink-0" />;
                            badgeContent = (
                              <Badge variant="destructive" className="text-xl md:text-2xl px-4 py-2">
                                ✗ إجابتك
                              </Badge>
                            );
                          }
                            ...
                            >
                              <div className="flex flex-col gap-5">
                                <div className="flex items-center gap-5">
                                  {iconElement || <div className="w-8 h-8 md:w-10 md:h-10"></div>}
                                  <span className={`${textColor} text-xl md:text-2xl font-bold break-words flex-1 leading-relaxed`}>
                    ...
                    {question?.explanation && (
                      <div className="bg-blue-50 border-r-5 border-blue-600 p-6 md:p-8 rounded-3xl mt-8">
                        <div className="flex items-start gap-5">
                          <Lightbulb className="w-10 h-10 md:w-12 md:h-12 text-blue-600 flex-shrink-0 mt-1" />
                          <div>
                            <h6 className="font-extrabold text-blue-900 text-2xl md:text-3xl mb-4 flex items-center gap-3">
                              <span>💡</span> شرح تفصيلي:
                            </h6>
                            <p className="text-blue-800 text-xl md:text-2xl break-words leading-relaxed">
                              {question.explanation}
                            </p>
                          </div>
                        </div>
                    ...
                    {!userAnswer.is_correct && (
                      <div className="bg-yellow-50 border-r-5 border-yellow-600 p-6 md:p-8 rounded-3xl mt-8">
                        <div className="flex items-start gap-5">
                          <AlertCircle className="w-10 h-10 md:w-12 md:h-12 text-yellow-600 flex-shrink-0 mt-1" />
                          <div>
                            <h6 className="font-extrabold text-yellow-900 text-2xl md:text-3xl mb-4 flex items-center gap-3">
                              <span>🎯</span> نصيحة للتحسين:
                            </h6>
                            <p className="text-yellow-800 text-xl md:text-2xl break-words leading-relaxed">
